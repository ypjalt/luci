...
function index()
	...
	entry({"admin", "usbapps", "vsftpd"},
		alias("admin", "usbapps", "vsftpd", "general"),
		_("FTP Server"))

	entry({"admin", "usbapps", "vsftpd", "general"},
		cbi("vsftpd/general"),
		_("General Settings"), 10).leaf = true

	entry({"admin", "usbapps", "vsftpd", "users"},
		cbi("vsftpd/users"),
		_("Virtual Users"), 20).leaf = true

	entry({"admin", "usbapps", "vsftpd", "anonymous"},
		cbi("vsftpd/anonymous"),
		_("Anonymous User"), 30).leaf = true

	entry({"admin", "usbapps", "vsftpd", "log"},
		cbi("vsftpd/log"),
		_("Log Settings"), 40).leaf = true

	entry({"admin", "usbapps", "vsftpd", "item"},
